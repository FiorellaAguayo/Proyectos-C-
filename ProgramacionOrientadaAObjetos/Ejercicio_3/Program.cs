...
namespace Ejercicio_3
{
    internal class Program
    {
        ...
        {
            Student firstStudent = new Student("Fiorella", "Aguayo", 12345678);
            Student secondStudent = new Student("Lara", "Cross", 87654321);
            Student thirdStudent = new Student("Juan", "Pepe", 13578642);

            List<Student> listStudents = new List<Student>() { firstStudent, secondStudent, thirdStudent };

            Random rand = new Random();

            foreach (Student student in listStudents)
            {
                student.SetNoteFirstPartial(rand.Next(1, 10));
                student.SetNoteSecondPartial(rand.Next(1, 10));
